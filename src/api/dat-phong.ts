...
const DAT_PHONG = {
    getAll: () => apiInstance.get<{ statusCode: number; content: Booking[] }>("/dat-phong"),
    getById: (id: number) =>
        apiInstance.get<{ statusCode: number; content: Booking }>(`/dat-phong/${id}`),
    getByUser: (maNguoiDung: number) =>
        ...
        apiInstance.post<{ statusCode: number; content: Booking }>("/dat-phong", data),
    update: (id: number, data: Partial<CreateBookingPayload>) =>
        apiInstance.put<{ statusCode: number; content: Booking }>(`/dat-phong/${id}`, data),
    delete: (id: number) => apiInstance.delete<{ statusCode: number }>(`/dat-phong/${id}`),
};
